...
| ----------------------- | ------------------------------------------------- | ------------------------------------- | ----------------------------- |
| Any browser / iPad now  | `apps/web/dist/molecular-cad.html` (single file)  | RDKit WASM (embedded)                 | built by `pnpm build` and CI  |
...
| Windows / macOS / Linux | Tauri 2 shell (`apps/desktop`)                    | RDKit WASM; Python API as a sidecar   | scaffold committed; build on a machine with the Tauri prerequisites |

## Single-file build (what you have been testing)

`pnpm build` writes `apps/web/dist/molecular-cad.html` (≈10 MB): the app, the RDKit glue and the
...
`apps/web/public/` carries `manifest.webmanifest`, the icons (regenerate with
`python3 apps/web/scripts/make-icons.py`) and `sw.js`, a cache-first service worker that
...
```bash
pnpm install
pnpm --filter @molecular-cad/desktop icons        # generates src-tauri/icons/* from icon-1024.png
...
- Tag; CI attaches `molecular-cad.html`; build installers on the platform runners
