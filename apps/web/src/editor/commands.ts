...
import {
  addAtom,
  addBond,
  attachFragment as attachFragmentToMolecule,
  bondBetween,
  bondInRing,
  getAtom,
  getBond,
...
export function attachFragment(mol: Molecule, anchorId: AtomId, fragment: FragmentTemplate): CommandResult {
  ...
  let attached: ReturnType<typeof attachFragmentToMolecule>;
  try {
    attached = attachFragmentToMolecule(mol, anchorId, fragment);
  } catch (e) {
    throw new CommandError(e instanceof Error ? e.message : String(e));
  }
  return { molecule: attached.molecule, label: `Attach ${fragment.name} to ${anchor.element}${anchorId}`, selection: { atoms: [attached.attachedAtomId], bonds: [] }, tidy: true };
}
